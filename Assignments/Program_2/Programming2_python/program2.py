...
import sys
# Run this file
# python3 program2.py <number of elements in the database> <file 1> <file 2> ...
...
class LockManger:
    ...
    def check_x_lock(self, k, tid):  # check like  X-lock(A) in the db
        ...
        for index, t_locks in enumerate(transactions):  # [1,2] means t1, t2 dbindex= 0, t_locks = 1
            if tid != index:
...
# print(T1a[0].Display())
# print(T1a[0].local)
# print(T1a[0].commands)
# print(len(T1a[0].commands))
# print(T1a[0].commands[0])
# print(C[2])
# execute_each_command(DB1a,T1a[0],C[2],1)
# T1a[0].Display()
# print(T1a[0].commands)
# C[2].ReleaseAll(1)
# DB1a.Print()
...
# t1_order = deque([])
# t1_order.append(t)
# execute_each_command(DB,t,L,1,t1_order)

# detect deadlock
def deadlock(dl, granted: int, T: list[Transaction], tid):  # index 0 1 2 3 4
    for i, j in enumerate(T):  # F
        if j.Finished() == True:
            dl[i] = False  #
...
if __name__ == '__main__':
    ...
    while loading(T):
        ...
        if not t.Finished():
            granted = execute_each_command(DB, t, LM, t_index)
            print("<"+ str(k[t_index]-len(t.commands))+">")
            # k=k+1
            if deadlock(dl, granted, T, t_index):
                print("Deadlock")
                break
        else:
